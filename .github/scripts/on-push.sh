...
if [ "$BUILD_PIO" -eq 0 ]; then
	...
	if [ ! "$OS_IS_WINDOWS" == "1" ]; then
		...
		git clone https://github.com/MDechert/ESPAsyncWebServer "$ARDUINO_USR_PATH/libraries/ESPAsyncWebServer" > /dev/null 2>&1
	...
	if [[ "$OSTYPE" != "cygwin" ]] && [[ "$OSTYPE" != "msys" ]] && [[ "$OSTYPE" != "win32" ]]; then
		echo "Installing ESPAsyncWebServer ..."
		python -m platformio lib -g install https://github.com/MDechert/ESPAsyncWebServer.git > /dev/null 2>&1
		git clone https://github.com/MDechert/ESPAsyncWebServer "$HOME/ESPAsyncWebServer" > /dev/null 2>&1
